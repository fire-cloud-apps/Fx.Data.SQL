...
namespace Fx.Data.SQL.xUnit
{
    ...
    public class SQLServerEntityServiceTest
    {
        ...
        {
            var userDetails = _entityService.Count(_dbName, _tblName, new List<Filter>()
            {
                new Filter()
                {
                    And = new List<And>()
        ...
        {
            var resultSet = _entityService.Exists(_dbName, _tblName, new List<Filter>()
            {
                new Filter()
                {
                    And = new List<And>()
        ...
        {
            var userDetails = _entityService.GetById("DMS", "User", "1");
            string json = JsonSerializer.Serialize(userDetails);
            json.ShouldNotBeEmpty();
        ...
        {
            var userDetails = _entityService.GetSingle("DMS", "User", new Helpers.Conditions()
            {
                Condition = "gt",
                Field = "ExpiresIn",
                Value = "10"
        ...
        public void ExecuteNonQuery_Test()
        {
            ...
            var userDetails = _entityService.ExecuteNonQuery("DMS", query);
